...
plt.title('Survival Rate Variation Across Passenger Calsses')
plt.xlabel('Pclass')
...
plt.bar(survived_count.index, survived_count,
        color = ['mediumorchid', 'darkvioet', 'indigo'])
plt.title('Survived Count by Embarked Port on Titanic')
...
# 성별에 따른 생존자의 수 계산
survived_count = titanic[titanic['Survived'] == 1]['Sex'].value_count()
print(survived_count)

# 수평 막대 그래프 그리기
bars = plt.barh(survived_count.index, survived_count, color=['darkturquoise', 'salmon'])
plt.title('Survived Count by Gender on Titanic')
plt.xlabel('Count')
plt.ylabel('Gender')
...
plt.pie(survived_count, labels=['Not Survived', 'Survived'], color=['orange', 'gold'],
        autopct='%0.1f%%', startangle=90, shadow=True, explode=(0, 0.1))

plt.title('Survival Distributon on the Titanic')
